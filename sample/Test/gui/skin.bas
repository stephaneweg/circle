...
function Skin.CreateFromImage(img as GImage ptr,count as unsigned integer,lw as integer,rw as integer,th as integer, bh as integer) as Skin ptr
    if img<>0 then
        dim result as Skin Ptr = cptr(Skin ptr,allocate(sizeof(Skin)))
        if result<>0 then
            ...
            result->SkinWidth = img->Width
            result->SkinHeight = img->Height/count
...
sub Skin.ApplyColor(c as unsigned integer,all as unsigned integer)
    if (all=0) then
        ...
        this.Image->FillRectangleAlphaHalf(0,0,this.Image->Width-1,this.Image->Height-1,c)
    end if
end sub
    
sub Skin.DrawOn(target as GImage ptr,num as unsigned integer,x as integer,y as integer,w as integer,h as integer,transparent as integer)
    dim i as integer
    ...
    if (oHeight>0 and oWidth>0) then
        ...
        target->FillRectangle(x+this.LeftWidth,y+this.TopHeight,x+w-this.RightWidth-1,y+h-this.BottomHeight-1,this.Image->Buffer[p] )
    end if
